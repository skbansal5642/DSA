...
class TreeOperations {
    ...
    public void deleteNode(int num) {
        ...
        // find if node present
        // get that node in temp
        ...
        // if its last node:
            // just delete it
        // else delete the last node and set the last node data to deleted node.
        int newData = lastNode.data;
        if (deleteNode.data != newData) {
            deleteNode.data = newData;
        }
        // delete last node
        deleteLastNode(root,newData);
    }

    private Boolean deleteLastNode(Node root, int num) {
        Boolean isFound = false;
        if (root.left != null && root.left.data == num) {
            root.left = null;
            isFound = true;
        } else if (root.right != null && root.right.data == num) {
            root.right = null;
            isFound = true;
        }
        if (!isFound && root.left != null) {
            isFound = deleteLastNode(root.left, num);
        }
        if (!isFound && root.right != null) {
            isFound = deleteLastNode(root.right, num);
        }
        return isFound;
    }
